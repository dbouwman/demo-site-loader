...
export default Ember.Service.extend({
  ...
  getSite() {
    ...
    if(!this.get('isFastBoot') && !this.get('showBoxChecked') && siteStore && siteStore.site){
      ...
      .then((fsResponse)=>{
        ...
        if(fsResponse.features.length){
          ...
          return Ember.RSVP.hash({
              item:this.get('itemsService').getById(domainInfo.siteId),
              data:this.get('itemsService').getDataById(domainInfo.siteId)
            })
            .then((result)=>{
